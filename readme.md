...
## 1) Gagal koneksi Supabase

Gejala:
- Log `Gagal connect ke Supabase`.
- Endpoint mengembalikan status error database offline.

Solusi:
- Validasi `SUPABASE_URL` dan `SUPABASE_KEY`.
- Pastikan project Supabase aktif dan key masih valid.

## 2) Login admin selalu gagal

Gejala:
- Username/password benar tapi tetap ditolak.

Solusi:
- Pastikan `password_hash` di tabel `admins` berformat bcrypt (bukan plain text).
- Buat ulang akun admin via `created_admin.py`.

## 3) Fitur CRM MTProto gagal

Gejala:
- Error `API_ID`/`API_HASH` tidak ditemukan.
- Verifikasi OTP gagal.

Solusi:
- Isi `API_ID` & `API_HASH` valid dari my.telegram.org.
- Pastikan session string tersimpan di tabel `crm_telegram_sessions`.

## 4) Bot tidak berjalan

Gejala:
- Startup crash dengan error `BOT_TOKEN missing`.

Solusi:
- Isi `BOT_TOKEN` valid.
- Jika ingin run tanpa bot, sesuaikan lifecycle `main.py` agar task bot tidak dipaksa start.

## 5) Port conflict

Gejala:
- `Address already in use` saat menjalankan server.

Solusi:
- Ganti port uvicorn atau hentikan proses yang memakai port 8000.
